...
# 조치 재확인(/api/ai/verify-action)은 ai_verify(8003)가 처리한다.
# 탐지(ai_server)는 8002라 여기로 넣으면 404가 난다.
AI_VERIFY_URL = os.getenv("AI_VERIFY_URL", "http://127.0.0.1:8003").rstrip("/")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            response = await client.post(
                f"{AI_VERIFY_URL}/api/ai/verify-action",
                data=data,
                files=files,
                timeout=20.0,
            )
